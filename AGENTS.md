...
- Find the WBSB title input by semantic attributes such as `aria-label="タイトル"` or `placeholder="Title"` instead of a fixed XPath, so page layout changes do not break title reads or writes.
